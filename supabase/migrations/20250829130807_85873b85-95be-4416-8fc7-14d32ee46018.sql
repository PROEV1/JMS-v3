-- Fix the ambiguous column reference in log_stock_request_audit function
...
DECLARE
  ...
  search_pattern TEXT;  -- Renamed from 'pattern' to avoid ambiguity
BEGIN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    ...
    WHERE (
      ...
      -- Pattern-based matching (fixed ambiguous reference)
      OR EXISTS (
        SELECT 1 FROM unnest(search_patterns) AS pattern_item
        WHERE t.reference ILIKE '%' || pattern_item || '%'
        OR t.notes ILIKE '%' || pattern_item || '%'
      )
