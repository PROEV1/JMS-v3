...
-- 1) Add partner-level flags
ALTER TABLE public.partners
  ADD COLUMN IF NOT EXISTS client_payment_required boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS client_agreement_required boolean NOT NULL DEFAULT true;

-- 2) Update status calculation to honor partner flags
...
BEGIN
  ...
  -- Next step if not scheduled yet
...
-- 3) Recalculate partner job statuses (so existing imports update immediately)
UPDATE public.orders
SET status_enhanced = public.calculate_order_status(orders)
