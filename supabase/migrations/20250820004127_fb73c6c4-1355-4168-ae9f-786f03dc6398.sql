-- Fix the order status calculation to prioritize engineer signoff over scheduled date
...
BEGIN
  ...
  -- PRIORITY: Check for engineer signoff first (most important status)
  IF has_engineer_signoff THEN
  ...
  -- Scheduled and in-progress states (only if engineer hasn't signed off)
