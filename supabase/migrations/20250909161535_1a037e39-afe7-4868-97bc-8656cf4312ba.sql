-- Fix the get_schedule_status_counts_v2 function with correct logic matching the list pages
...
BEGIN 
  WITH counts AS (
    SELECT 
      ...
      -- Date Rejected: orders with status 'date_rejected'
      COUNT(*) FILTER (
        WHERE o.status_enhanced = 'date_rejected'
      ) AS date_rejected,
