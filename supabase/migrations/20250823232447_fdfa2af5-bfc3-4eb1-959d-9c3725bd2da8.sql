-- Update the calculate_order_status_final function to consider client_survey_required for partner jobs
...
BEGIN
  ...
  -- Determine if survey is required for this order
  survey_required_for_order := COALESCE(order_row.survey_required, true);
  
  -- For partner jobs, also check the partner's survey requirement setting
  IF order_row.is_partner_job IS TRUE AND order_row.partner_id IS NOT NULL THEN
    ...
    -- If partner doesn't require surveys, override the order-level setting
    IF survey_required_for_order IS FALSE THEN
      survey_required_for_order := false;
    END IF;
  END IF;

  -- Survey gating applies only before a date is scheduled and only if survey is required
