-- Fix calculate_order_status_final function to handle parts required workflow correctly
-- The issue: scheduling_suppressed was preventing standard quotes with parts_required from entering awaiting_parts_order status
...
BEGIN
  ...
  -- FIXED: Allow parts workflow for standard quotes with parts required, even if scheduling suppressed
  IF (order_row.scheduling_suppressed IS TRUE) THEN
    -- Special case: If this is a standard quote with parts required and not yet ordered, 
    -- allow it to flow through parts workflow despite scheduling suppression
    IF order_row.quote_type = 'standard' AND 
       COALESCE(order_row.part_required, false) = true AND 
       COALESCE(order_row.parts_ordered, false) = false AND
       order_row.scheduling_suppressed_reason = 'awaiting_quotation' THEN
      -- Continue to normal workflow logic below
    ELSE
      -- Otherwise, honor the scheduling suppression
      RETURN 'on_hold_parts_docs'::order_status_enhanced;
    END IF;
  END IF;

  -- Partner status overrides
  IF (order_row.partner_status IN ('ON_HOLD','SWITCH_JOB_SUB_TYPE_REQUESTED','WAITING_FOR_OHME_APPROVAL')) THEN
    RETURN 'on_hold_parts_docs'::order_status_enhanced;
  END IF;
