...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_offers_order_id_status_expires 
ON job_offers (order_id, status, expires_at) 
WHERE status IN ('pending', 'accepted');

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_job_offers_status_expires_active 
ON job_offers (status, expires_at) 
WHERE status = 'pending' AND expires_at > now();

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_status_enhanced_scheduling 
ON orders (status_enhanced, scheduling_suppressed) 
WHERE scheduling_suppressed = false;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_scheduled_date_status 
