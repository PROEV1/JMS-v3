-- Update get_schedule_status_counts_v2 to count date_offered based on status_enhanced field
...
BEGIN 
  WITH counts AS (
    SELECT 
      ...
      -- Date Offered: orders with status_enhanced = 'date_offered'
      COUNT(*) FILTER (
      ...
      -- Date Rejected: orders with status_enhanced = 'date_rejected' OR orders that have rejected offers but NO active offers AND not already scheduled/completed
      (
        -- Count orders explicitly marked as date_rejected
        SELECT COUNT(*) 
        FROM orders o_rejected
        WHERE o_rejected.status_enhanced = 'date_rejected'
          AND o_rejected.scheduling_suppressed = false
          AND o_rejected.scheduled_install_date IS NULL
      ) + (
        -- Count orders with rejected offers and no active offers
        ...
        WHERE jo_rejected.status = 'rejected'
          AND o_rejected.scheduling_suppressed = false
          AND o_rejected.status_enhanced NOT IN ('scheduled', 'in_progress', 'install_completed_pending_qa', 'completed', 'cancelled', 'date_rejected')
          AND NOT EXISTS (
