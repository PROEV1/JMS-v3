-- Fix order status calculation for deposit payments
-- The issue is that the function needs to properly handle deposit vs full payment
...
BEGIN
  ...
  -- CRITICAL FIX: Override the base status payment logic to use deposit logic
  IF base_status = 'awaiting_payment'::order_status_enhanced THEN
    IF payment_required AND NOT deposit_fully_paid THEN
      RETURN 'awaiting_payment'::order_status_enhanced;
    ELSE
      -- Deposit is paid, check agreement
      IF agreement_required AND NOT has_agreement THEN
        RETURN 'awaiting_agreement'::order_status_enhanced;
      ELSE
        RETURN 'awaiting_install_booking'::order_status_enhanced;
...
-- Update the trigger to ensure it recalculates status properly
CREATE OR REPLACE FUNCTION public.update_order_status_on_payment_change()
RETURNS TRIGGER AS $$
BEGIN
  -- Only process if amount_paid, deposit_amount, or agreement_signed_at changed
  IF TG_OP = 'UPDATE' AND (
    OLD.amount_paid IS DISTINCT FROM NEW.amount_paid OR
    OLD.deposit_amount IS DISTINCT FROM NEW.deposit_amount OR
    OLD.agreement_signed_at IS DISTINCT FROM NEW.agreement_signed_at
  ) THEN
    -- Recalculate status_enhanced using the fixed function
...
-- Manually update the specific order that's stuck
UPDATE public.orders 
SET status_enhanced = public.calculate_order_status_final(orders.*),
    updated_at = now()
WHERE id = 'b6ca8279-c317-4e24-aac1-2e6200c52dd9';