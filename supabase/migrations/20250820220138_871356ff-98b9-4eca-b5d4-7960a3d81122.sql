...
-- 2) Parent table for stock requests
...
-- Engineers: create/select/update own requests while submitted
...
-- Lines follow parent access; engineers manage while submitted
...
-- 7) Add engineer_id to inventory_locations so vans can be assigned
alter table public.inventory_locations
  add column if not exists engineer_id uuid references public.engineers(id);

create index if not exists idx_inventory_locations_engineer
